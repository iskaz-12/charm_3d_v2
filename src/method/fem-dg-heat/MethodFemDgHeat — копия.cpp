...
namespace charm {
	...
	void MethodFemDgHeat::convertToParam(int i, Point pt, Prim& p)
	{
	...
	*/
		...
		/*
    		else if (BASE_FUNC_COUNT == 6)
    		{
        		switch(i) {
            			...
            			case 3:
                			return (pt.x - mesh->cells[iCell].c.x) * (pt.x - mesh->cells[iCell].c.x) / (mesh->cells[iCell].HX * mesh->cells[iCell].HX);
            			case 4:
                			return (pt.y - mesh->cells[iCell].c.y) * (pt.y - mesh->cells[iCell].c.y) / (mesh->cells[iCell].HY * mesh->cells[iCell].HY);
            			case 5:
                			return (pt.x - mesh->cells[iCell].c.x) * (pt.y - mesh->cells[iCell].c.y) / (mesh->cells[iCell].HX * mesh->cells[iCell].HY);
        		}
    		}
		*/
    		else {
	...
	void MethodFemDgHeat::init() {
		...
		//	TMAX и TAU читаются из файла task.yaml

		//	UPDATE от 28.12.2022 - пробую уменьшить TAU
		//	БЫЛО: TAU     = 1.e-5;

		//	UPDATE от 02.01.2023 - меняем параметры времени
		//	TAU выставляю меньше, чем в статье...
		...
		//	Базисных функций у нас будет 4
		//	Или лучше брать 3???
    		BASE_FUNC_COUNT = 4;

		//	Вроде, это всё подсчитывается в Mesh.h
		
		//	количество точек Гаусса, используемых для вычисления объёмных интегралов
    		GP_CELL_COUNT = 8;

		//	количество точек Гаусса, используемых для вычисления поверхностных интегралов
    		GP_EDGE_COUNT = 3;
    		C11 = 1.0;
		
		cellGP = new Point*[cN];
    		cellGW = new double*[cN];
    		cellJ = new double[cN];
    		edgeGP = new Point*[fN];
    		edgeGW = new double*[fN];
    		edgeJ = new double[fN];
	...
	void MethodFemDgHeat::calcMassMatrix() {
    		for (int ic = 0; ic < mesh->cells.size(); ic++) {
        		double **mA = A[ic];
        		double **invA_ = invA[ic];
        		for (int i = 0; i < BASE_FUNC_COUNT; i++) {
            			for (int j = 0; j < BASE_FUNC_COUNT; j++) {
                			mA[i][j] = 0.0;
                			for (int iGP = 0; iGP < GP_CELL_COUNT; iGP++) {
                    				//mA[i][j] += cellGW[ic][iGP] * getF(i, ic, cellGP[ic][iGP]) * getF(j, ic, cellGP[ic][iGP]);
						mA[i][j] += mesh->cells[ic].gw[iGP] * getF(i, ic, mesh->cells[ic].gp[iGP]) * getF(j, ic, mesh->cells[ic].gp[iGP]);
                			}
                			//mA[i][j] *= cellJ[ic];
					mA[i][j] *= (mesh->cells[ic].dh[0]*mesh->cells[ic].dh[1]*mesh->cells[ic].dh[2]) / 8.;
    	...
    	// поверхностные интегралы (в нашем случае - объёмные???)
    		for (int i = 0; i < mesh->cells.size(); i++){
        		for(int j = 0; j < BASE_FUNC_COUNT; j++) {
					...
					tmpIntQx += mesh->cells[i].gw[z] * getT(i, cellGP[i][z]) * getDfDx(j, i, cellGP[i][z]);
                			tmpIntQy += mesh->cells[i].gw[z] * getT(i, cellGP[i][z]) * getDfDy(j, i, cellGP[i][z]);

					tmpIntQz += mesh->cells[i].gw[z] * getT(i, cellGP[i][z]) * getDfDz(j, i, cellGP[i][z]);
            			}
	...
	void MethodFemDgHeat::calcDiffusionVol() {
    		for (int i = 0; i < mesh->cells.size(); i++){
        		for(int j = 0; j < BASE_FUNC_COUNT; j++) {
            			double tmpIntT = 0.0;
            			for (int z = 0; z < GP_CELL_COUNT; z++) {
                			tmpIntT += mesh->cells[i].gw[z] * getQx(i, mesh->cells[i].gp[z]) * getDfDx(j, i, mesh->cells[i].gp[z]);
                			tmpIntT += mesh->cells[i].gw[z] * getQy(i, mesh->cells[i].gp[z]) * getDfDy(j, i, mesh->cells[i].gp[z]);

					tmpIntT += mesh->cells[i].gw[z] * getQz(i, mesh->cells[i].gp[z]) * getDfDz(j, i, mesh->cells[i].gp[z]);
            			}
