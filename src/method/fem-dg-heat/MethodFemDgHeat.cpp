    	...
    	// поверхностные интегралы (в нашем случае - объёмные???)
    		for (int i = 0; i < mesh->cells.size(); i++){
        		for(int j = 0; j < BASE_FUNC_COUNT; j++) {
					...
					tmpIntQx += mesh->cells[i].gw[z] * getT(i, mesh->cells[i].gp[z]) * getDfDx(j, i, mesh->cells[i].gp[z]);
                			tmpIntQy += mesh->cells[i].gw[z] * getT(i, mesh->cells[i].gp[z]) * getDfDy(j, i, mesh->cells[i].gp[z]);

					tmpIntQz += mesh->cells[i].gw[z] * getT(i, mesh->cells[i].gp[z]) * getDfDz(j, i, mesh->cells[i].gp[z]);
            			}
